...
interface PedidoCardProps { order: Order }
        ...
        <div id={bodyId}>
                ...
                <li key={item.product_id} className="flex items-center gap-4 px-5 py-4">
                  <div className="relative h-14 w-14 flex-shrink-0 overflow-hidden rounded-lg bg-gray-100">
                    {product?.thumbnail_url ? (
                      <Image src={product.thumbnail_url} alt={product.title ?? 'Atividade'} fill sizes="56px" className="object-cover" />
                    ) : (
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="truncate text-sm font-medium text-gray-900">{product?.title ?? 'Atividade'}</p>
                    {isPaid && <p className="mt-0.5 text-xs text-gray-400">PDF disponivel para download</p>}
                  </div>
                  {isPaid && (
                    <a
          ...
          {isPaid && (
            <div className="flex items-center justify-between border-t border-gray-100 px-5 py-3">
              <span className="text-xs text-gray-400">Acesso permanente aos PDFs comprados</span>
